...
def search_jobs(query, model="gpt-4o", max_tokens=4000):
    ...
    print(f"Search returned {len(result_text)} characters of results")
...
def save_results(jobs, output_dir=None):
    ...
    try:
        ...
        print(f"Results saved to Markdown: {md_path}")
    except ImportError:
...
def main():
    ...
    print(f"  - Tokens: ~{tokens_estimate:,} tokens")
    print(f"  - Cost: ~${cost_estimate:.4f}")

    # Check budget constraint if specified
    if args.budget and cost_estimate > args.budget:
        print(f"⚠️ ERROR: Estimated cost ${cost_estimate:.4f} exceeds budget ${args.budget:.2f}")
        print("Exiting without running search")
        return 1

    print(f"Searching for {args.majors} major company jobs and {args.startups} startup jobs using {args.model}")
